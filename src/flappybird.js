...
// ===== Sound effects =====
// Make sure these files exist next to this JS file in /src
...
// Helper to allow overlapping sounds (clone each time we play)
...
function onUserFlap() {
  if (gameState === "start") {
    ...
    playSound(flapSound); // 🔊 flap on first start
  } else if (gameState === "running") {
    bird.vy = FLAP_STRENGTH;
    playSound(flapSound); // 🔊 normal flap
    ...
    resetGame();
    gameState = "running";
    playSound(flapSound); // 🔊 optional: flap on restart
  }
}
...
function updateScore() {
  for (const pipe of pipes) {
    if (!pipe.scored && pipe.x + pipe.width < bird.x) {
      pipe.scored = true;
      score++;
      playSound(scoreSound); // 🔊 scoring sound
...
function drawBackground() {
  if (bgImg.complete) {
    const patternWidth = bgImg.width || WIDTH;
...
function gameLoop(timestamp) {
  ...
  if (gameState === "start") {
    ...
    // Collisions
    if (checkCollisions()) {
      playSound(hitSound); // 🔊 collision sound
      gameState = "gameover";
